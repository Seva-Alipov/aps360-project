#!/usr/bin/env python3
"""
test_doom_vs_ghidra.py

Compare your model vs Ghidra on the DOOM test set:

For each stem in ../test_data:
  - Load assembly:   ../test_data/<stem>.s   (model input)
  - Load original C: ../test_data/<stem>.c   (ground truth from Doom)
  - Load Ghidra C:   ../test_data/<stem>.ghidra.c (from generate_ghidra.py)
...
DOOM_DATA_DIR = (ROOT / "test_data").resolve()
...
# ---- Levenshtein similarity (token-level) ----
...
def main() -> None:
    print("[test_doom_vs_ghidra] Loading model from", MODEL_DIR)
    tokenizer, model, device = load_model()

    if not DOOM_DATA_DIR.exists():
        print(f"[test_doom_vs_ghidra] No ../test_data dir at {DOOM_DATA_DIR}")
        return

    all_c_files = sorted(DOOM_DATA_DIR.glob("*.c"))
    # Ignore *.ghidra.c – only want the original C files
    c_files = [p for p in all_c_files if not p.name.endswith(".ghidra.c")]

    print(f"[test_doom_vs_ghidra] Found {len(c_files)} original .c files in {DOOM_DATA_DIR}")
    ...
    for idx, c_path in enumerate(c_files, 1):
        stem = c_path.stem
        s_path = DOOM_DATA_DIR / f"{stem}.s"
        ghidra_path = DOOM_DATA_DIR / f"{stem}.ghidra.c"

        if not (s_path.exists() and ghidra_path.exists()):
            print(f"[{idx}] SKIP (missing .s or .ghidra.c) for stem {stem}")
            continue

        print(f"\n[{idx}/{len(c_files)}] stem={stem}")
    ...
    if used == 0:
        print("[test_doom_vs_ghidra] No usable samples found.")
        return

    avg_gh = sum(ghidra_sims) / used
    avg_model = sum(model_sims) / used

    print("\n[test_doom_vs_ghidra] Summary over", used, "samples:")
